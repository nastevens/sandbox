...
/*
 *  This file is a modified version of the file EventListenerList.java from the
 *  package javax.swing.event, which is under the Apache License and is
 *  distributed by the Apache Software Foundation (ASF).
 * 
...
package com.qiggroup.javappp.util;
...
public class EventListenerList {
    ...
    public synchronized <T extends java.util.EventListener> void remove(
            final Class<T> listenerClass, final T listener) {
        if(listener == null) {
            return;
        }
        ...
        for(int i = listenerList.length - 1; i > 0; i -= 2) {
            if(listenerClass == listenerList[i - 1]
    ...
    public <T extends java.util.EventListener> T[] getListeners(
            final Class<T> listenerClass) {
        int numClassListeners = getListenerCount(listenerClass);
        T[] listeners = (T[]) (Array.newInstance(listenerClass,
                numClassListeners));
    ...
    /*
     * The format of the string is based on 1.5 release behavior which can be
     * revealed using the following code:
     * 
     * EventListenerList obj = new EventListenerList();
     * obj.add(BasicButtonListener.class, new BasicButtonListener(new
     * JButton())); System.out.println(obj.toString());
     */
