 ...
 *  set_fat_entry
 *  
 * Authors: Andy Kinley, Archana Chidanandan, David Mutchler and others.  March, 2004.
 **************************************************************************************/
 ...
 * read_sector
 *
 * Read the specified sector from the file system and store that sector in the given buffer
 *
 * sector_number:  The number of the sector to read (0, 1, 2, ...)
 * buffer:  The array into which to store the contents of the sector that is read
 *
 ...
 * write_sector
 *
 * Write the contents of the given buffer to the filesystem at the specified sector
 *
...
int get_fat_entry(int fat_entry_number, unsigned char* fat) {
    ...
    if (fat_entry_number & 0x0001) { // odd fat entry number, return yzw
        wx = (int) fat[offset];
        yz = (int) fat[offset + 1];
        return ( (yz << 4)  |  ( (wx & 0x00f0) >> 4));
    } else {                         // even fat entry number, return xuv
        uv = (int) fat[offset];
        wx = (int) fat[offset + 1];
...
void set_fat_entry(int fat_entry_number, int value, unsigned char* fat) {
    ...
    // Two FAT12 entries are stored into three bytes;
    // if these bytes are uv,wx,yz then the two FAT12 entries are xuv and yzw
    // Let 0a,bc denote the fat_entry_number, written as two bytes (high and low, respectively)

    a = value & 0x0f00;
    b = value & 0x00f0;
    c = value & 0x000f;

    if (fat_entry_number & 0x0001) { // odd fat entry number, change yzw to abc, i.e.,
        fat[offset]     = (unsigned char) ((c << 4)  |  (fat[offset] & 0x000f));       // wx = cx;
        fat[offset + 1] = (unsigned char) ((a >> 4)  |  (b >> 4));                     // yz = ab;

    } else {                         // even fat entry number, change xuv to abc, i.e.,
        fat[offset]     = (unsigned char) (b | c);                                     // uv = bc;
        fat[offset + 1] = (unsigned char) ((fat[offset + 1]  &  0x00f0)  |  (a >> 8)); // wx = wa;
    }
}
